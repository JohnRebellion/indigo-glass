...
const PAGE_FN = () => {
  ...
  /* Disabled controls. Raised in review: a restyle can leave a disabled
   * control looking enabled, or push it under the 3:1 floor that still
   * applies to a UI component's own boundary. `:visited` is deliberately NOT
   * measured — Chromium lies about it in getComputedStyle as an anti-history-
   * sniffing measure, so any number this script produced would be fiction. */
  const disabled = []
  for (const el of [...document.querySelectorAll('[disabled], [aria-disabled="true"]')].slice(0, 60)) {
    const leaf = [el, ...el.querySelectorAll('*')].find((n) =>
      [...n.childNodes].some((t) => t.nodeType === 3 && t.textContent.trim()))
    if (!leaf) continue
    const m = measure(leaf)
    if (!m) continue
    /* Disabled text is meant to be dimmer; 3:1 is the floor at which it still
     * reads as text rather than as a smudge. */
    if (m.ratio < 3) disabled.push({ el: sig(el), text: (leaf.innerText || '').trim().slice(0, 24), ...m })
  }

  /* Non-CSS colour: SVG paint survives a computed-style retint untouched, so
   * a brand-coloured inline icon can sit in an audit that reports "clean". */
  const svgPaint = new Map()
  for (const el of [...document.querySelectorAll('svg [fill], svg [stroke], svg')].slice(0, 400)) {
    const c = getComputedStyle(el)
    for (const prop of ['fill', 'stroke']) {
      ...
      /* Greys and near-greys are almost always intentional icon colour; a
         saturated value is a brand colour that escaped the retint. */
      const mx = Math.max(rgb.r, rgb.g, rgb.b), mn = Math.min(rgb.r, rgb.g, rgb.b)
      if (mx - mn < 40) continue
      const key = `${prop}=${v}`
      svgPaint.set(key, (svgPaint.get(key) || 0) + 1)
    }
  }
  ...
  return {
    ...
    disabledFailures: worst(disabled).slice(0, 8),
    svgBrandPaint: [...svgPaint.entries()].sort((a, b) => b[1] - a[1]).slice(0, 8).map(([k, v]) => `${v}x ${k}`),
