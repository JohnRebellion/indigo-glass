...
def emit_klassy_radius(t: dict) -> str:
    ...
    [ShadowStyle] comes from shadow.klassy - see the tokens file comment for
    why this zeroes the shadow rather than trying to approximate the "4px
    4px 0" hard ink shadow: Klassy's window shadow has no offset control,
    only size/strength/colour, so a soft shadow of any size still reads as
    a glow. Merge into ~/.config/klassyrc."""
    radius = t["radius"]["default"]
    sh = t["shadow"]["klassy"]
    lines = [
        ...
        "# (see tokens file: Klassy has no hard-offset shadow capability).",
        "",
        "[Windeco]",
