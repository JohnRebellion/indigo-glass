...
| Win11 accent `#A6C9A6` + dark mode | `registry/indigo-glass-accent.reg` | HKCU only, no admin |
...
Win11 host
  ...
  ├── PowerShell 7          Starship prompt, sage PSReadLine colors
  ├── VSCode                theme JSON + Claude Code webview patch
  ├── Edge browser          Stylus + Dark Reader, cloud-synced
  ├── System chrome         accent #A6C9A6 + dark mode via registry
