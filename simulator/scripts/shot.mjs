 ...
 * Needs the static server up (npx http-server build -p 4173 -s -c-1), or SIM_BASE_URL
 * pointing at a running `vite dev --port N`. */
...
await p.goto((process.env.SIM_BASE_URL ?? 'http://127.0.0.1:4173') + path); await p.waitForTimeout(500);
