...
| Wall | Why |
|---|---|
| **Dolphin file listing** | Painted by KDE Frameworks' `KItemListView`/`KItemListWidget` (`libKF6ItemViews`), compiled C++. **Re-measured live 2026-09-06 — the original wording here was wrong on two counts.** (1) The selected row now renders as an *outline*, not the filled rect this row used to describe, so the Tier C noun is correct. (2) The fix does **not** need a `libKF6ItemViews` patch: `kitemlistwidget.cpp` only hard-codes its own rounded, `lighter(110)` selection painting inside a `style()->name() == "breeze"` branch, and Klassy reports `"klassy"` (verified empirically: `QT_STYLE_OVERRIDE=klassy` -> `style().name() == 'klassy'`, `metaObject 'Breeze::Style'`). Dolphin therefore takes the `else` branch and delegates to `PE_PanelItemViewItem` — i.e. **our own** patched `Style::drawPanelItemViewItemPrimitive`. Two residual defects, both ours to fix in `config/klassy/tierc-outline.patch`: the stroke is **1px** where Tier C specifies 2px (`Helper::renderFocusRect` does `painter->setPen(outline)` with a bare `QColor`, so the pen defaults to width 1 — the patch sets `focusColor` but never a pen width), and the sampled ring colour is `#B7DDB7`, a `Highlight`-derived value rather than the `#F8F8F8` ring token the patch intends. Measured by pixel-sampling a live capture: exactly one pixel row at y=354 and y=373, `(183,221,183)`. `DecorationFocus=248,248,248` is set correctly in all seven colour groups; this path does not read it. The exact provenance of `#B7DDB7` is **not yet pinned** — it equals `Highlight.lighter(110)` byte-for-byte, but the only code performing that scale sits in the breeze-only branch that is not taken, and both an alpha-0.8-over-surface and an alpha-0.8-over-fill composite were computed and **refuted** (`#87A388`, and an out-of-gamut backdrop respectively). Resolve by instrumenting the paint path or by testing whether the ring colour changes with row current-ness, before patching the colour. The width defect is independent and actionable now. |
